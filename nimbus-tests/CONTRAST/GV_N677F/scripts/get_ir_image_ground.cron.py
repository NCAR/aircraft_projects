...
osm_file_name    = "latest_ir_tb.jpg"
num_imgs_to_get  = 2 # Script will backfill this many images for loops
...
#  Clean out older images (except for most recent so logic below will kick out of script)
count = 0   
for i in listing:
    if prefix in i and postfix in i:
        if count > 0:
...
try:
    ...
    if "CONTRAST" not in latest:
        command = "cp "+latest+" "+osm_file_name
    else:
        command = "cp "+latest+" "+osm_file_name2
    os.system(command)

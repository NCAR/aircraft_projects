...
# * * * * * /home/local/projects/scripts/avaps_proc.cron.py3
#
import os
import sys
import glob
...
####################  CONFIGURATION #######################################
send_Dfiles =  True
send_prodfiles = False
# The following probably won't change
Aspen_QC_exe = '/home/local/aspenqc/bin/run_Aspen-QC'
d_file_re = 'CAESAR*.nc'
...
for file in list:
    ...
    skewt_fn = file+'.svg'
    wmo_fn = file+'.wmo'
    cmd = Aspen_QC_exe + ' -i '+file+ ' -d 400:1050:-50:150 -g '+skewt_fn+' -w '+wmo_fn
    os.system(cmd)

...
$(function(){
	...
	setInterval("updateDsmDetails()", dsmdetailsUpdateInterval*1000);
	...
	$('#rebooter').click(function() {
		if ($('.ui-selected').size()) {
			...
			$('.ui-selected').each(function(idx, item) {
				...
				if (dsmID >= 100) { 
					$('#reboot-dialog')
						.html('<p><strong style="color:#600">'
							+'Can\'t reboot the server, please select one or more DSMs.'
							+'</strong></p>')
						.dialog('option', 'buttons', {
	...
	$(".sel").selectable({
		filter: 'tr',
		stop: function() {
			curDsmDetail = $('.ui-selected:first').attr('id').split("-", 1);
			updateDsmDetails();
		}
	});
...
//ajax function to call commands.php on server
	$('#reboot-dialog')
		...
		.dialog('option', 'buttons', { 
			...
			'Yes': function(){
				var sOpen = $('#keepTermOpen').attr('checked');
				$.get("commands.php", {"command":name, "action":action, "stayOpen":sOpen });
				$(this).dialog('close'); 
			}
	 }).dialog('open');
}
function updateDsmDetails(){
	if (curDsmDetail) {
		var arg = (curDsmDetail >= 100) ?
		xmlrpcItems[curDsmDetail-100].xmlrpcTag : dsmlist[ curDsmDetail ];

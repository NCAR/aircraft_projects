...
# ./run_ddclient.sh ppp0 38400 12.42.105.41 12.42.104.9 ppp0
#
# set -x

if [ $# -lt 4 ]; then
    echo "$0 if baud local_IP remote_IP if"
    echo "example: $0 ppp0 38400 12.42.105.41 12.42.104.9 ppp0"
...
for (( ncheck=0; ncheck < 5; ncheck++ )); do
    ...
    if [ -n "$dname" ]; then
        ...
        logger -t ddclient "Don't recognize hostname `hostname`, can't check ddclient success"
    fi
